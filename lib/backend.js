	...
	function getShouts(sids, isNew, callback) {
		...
		db.getObjects(keys, function(err, shouts) {
			...
			User.getMultipleUserFields(uids, ['username', 'userslug', 'picture', 'status'], function(err, usersData) {
				...
				async.map(shouts.map(function (v, i) {
						return { value: v, index: i };
					})
					, function(shout, next) {
					if (parseInt(shout.value.deleted, 10) === 1) {
						return next(null);
					}

					userData = usersData[uids.indexOf(parseInt(shout.value.fromuid))];
					userData.uid = shout.value.fromuid;

					Backend.parse(shout.value.content, userData, isNew, function(err, s) {
						shout.value.user = s.user;
						shout.value.isNew = s.isNew;
						shout.value.content = s.content;
						shout.value.sid = sids[shout.index];

						next(null, shout.value);
					});
				}, callback);
			});
		});
