	...
	features = [
		{
			name: 'Gists',
			id: 'gist',
			description: "Easily create Gists. (Please note, language file not loaded)",
			icon: 'fa-github-alt',
			button: 'Create Gist',
			enabled: true
		},
		{
			name: 'Archive',
			id: 'archive',
			description: 'View older posts. (Please note, language file not loaded)',
			icon: 'fa-archive',
			button: 'View Archive',
			enabled: true
		},
		{
			name: 'Bugs',
			id: 'bug',
			description: 'Report bugs quickly. (Please note, language file not loaded)',
			icon: 'fa-bug',
			button: 'Report Bug',
			enabled: false
		}
...
//Dirty work around to translate stuff
features.forEach(function(feature){
	// Translate the text that is shown to the user
	
	translator.translate("[[shoutbox:" + feature.id + "_name]]", function(translated){
		feature.name = translated;
	});
	translator.translate("[[shoutbox:" + feature.id + "_description]]", function(translated){
		feature.description = translated;
	});
	translator.translate("[[shoutbox:" + feature.id + "_button]]", function(translated){
		feature.button = translated;
	});
});
